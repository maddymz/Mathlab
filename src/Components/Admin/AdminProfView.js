...
class AdminProfView extends Component {
  ...
  handleSubmit(event) {
    ...
    var students = JSON.parse(localStorage.getItem('professors'));
    const myObj = {
      ...
      if(flag === 0) {
        students.push(myObj);
        localStorage.setItem('professors', JSON.stringify(students));
        alert("Successfully added Professor"); 
        axios
        .post('http://localhost:3001/addProfessor', students)
        .then(() => { 
  ...
  render(){
    var addButton =  <RaisedButton label="Add Professor" primary={true} 
      style={  {marginTop: '200px',marginLeft: '400px',marginRight: '100px',marginBottom: '100px'}}/>;
    var deleteButton = <RaisedButton label="Delete Professor" primary={true} 
    ...
    var obj = JSON.parse(localStorage.getItem('professors'));
...
function Delete(context){
  ...
  localStorage.setItem('professors', JSON.stringify(students));
  axios
    .post('http://localhost:3001/deleteProfessor', students)
